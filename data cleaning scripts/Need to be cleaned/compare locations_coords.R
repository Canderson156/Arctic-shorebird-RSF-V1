...
check_g8 <- prism_polys_proj %>%
  filter(PlotX %in% g8_good_plots$PlotX) %>%
  arrange(PlotX)
...
GIS_check_g8 <- GIS_shapefiles %>%
  filter(Plot %in% check_g8$Plot) %>%
  arrange(Plot) %>%
  distinct(.keep_all = TRUE)



GIS_check_g8_dups <- GIS_check_g8%>%
  filter(allDuplicated(Plot) == TRUE)

GIS_check_g8  <- GIS_check_g8 %>%
  filter(allDuplicated(Plot) == FALSE)



## make a version of Isabel's plots that matches this one

PRISM_check_g8 <- prism_polys_proj %>%
  filter(Plot %in% GIS_check_g8 $Plot) %>%
  arrange(Plot)
...
dist <- st_distance(GIS_check_g8, PRISM_check_g8, by_element = TRUE)
dist <- set_units(dist, km)

dist2 <- round(as.numeric(dist))
PRISM_check_g8$dist <- dist2
check <- dist2[dist2 > 50]

###I should visualize these in GIS

st_write(PRISM_check_g8, "exported/PRISM_check_apr27.shp")
st_write(GIS_check_g8, "exported/GIS_check_apr27.shp")
